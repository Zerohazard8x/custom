...
printf '%s\n' \
'Select download option (defaults to 5 -> Unlimited quality) within 5 seconds:' \
'    1) Audio only' \
'    2) Video 480p' \
'    3) Video 720p' \
'    4) Video 1080p' \
...
read -t 5 -rp $'Enter choice [1-5] (default 5): ' choice
choice=${choice:-5}

# 4. Build format-specific flags
common_opts="--add-header Accept-Language:'en-US,en;q=0.5' \
--add-metadata --embed-thumbnail --write-subs \
--no-write-auto-subs \
--embed-subs \
--ignore-errors --sponsorblock-mark all --sponsorblock-remove sponsor,selfpromo,music_offtopic --no-simulate --extractor-args \"generic:impersonate\""
...
if [ "$choice" -ne 1 ] && [ -f ./transcribe.sh ]; then
    # Prompt for transcription with 5-second timeout, default to "N"
    read -t 5 -p "Transcribe? [y/N] " ans
    ans=${ans:-N}
