...
# With MEDIA shifted away, `for s; do` covers every subtitle argument
# without copying the list. `$srt` still names the first subtitle for
...
# Synchronize first so that the later cleanup and duration-limit pass operate
# on the final synchronized timings rather than having ffsubsync alter them
# afterward.
